/*
	Ring Bcrypt Library Install Script
	----------------------------------
	This script installs Ring Bcrypt library for the current platform.
...
cPackagePath = exefolder() + ".." + cPathSep + "tools" + cPathSep + "ringpm" + cPathSep + "packages" + cPathSep + "bcrypt"

# Construct the library path
cLibPath = cPackagePath + cPathSep + "lib" + cPathSep +
		cOSName + cPathSep + cArchName + cPathSep + cLibPrefix + "ring_bcrypt" + cLibExt

# Verify library exists
if not fexists(cLibPath)
	? colorText([:text = "Error: Ring Bcrypt library not found!", :color = :BRIGHT_RED, :style = :BOLD])
...
try
	...
	# Copy examples to the samples/UsingBcrypt directory
	cCurrentDir = currentdir()
	cExamplesPath = cPackagePath + cPathSep + "examples"
	cSamplesPath = exefolder() + ".." + cPathSep + "samples" + cPathSep + "UsingBcrypt"
	...
	# Create the UsingBcrypt directory
	...
	# Check if bcrypt.ring exists in the exefolder
	if fexists(exefolder() + "bcrypt.ring")
		# Remove the existing bcrypt.ring file
		remove(exefolder() + "bcrypt.ring")

		# Write the load command to the bcrypt.ring file
		write(exefolder() + "load" + cPathSep + "bcrypt.ring", `load "/../../tools/ringpm/packages/bcrypt/lib.ring"`)
	ok
	
	# Ensure the Ring2EXE libs directory exists
	if direxists(exefolder() + ".." + cPathSep + "tools" + cPathSep + "ring2exe" + cPathSep + "libs")
		# Write the library definition to the bcrypt.ring file for Ring2EXE
		write(exefolder() + ".." + cPathSep + "tools" + cPathSep + "ring2exe" + cPathSep + "libs" + cPathSep + "bcrypt.ring", getRing2EXEContent())
	ok

	? colorText([:text = "Successfully installed Ring Bcrypt!", :color = :BRIGHT_GREEN, :style = :BOLD])
	...
	? colorText([:text = "Error: Failed to install Ring Bcrypt!", :color = :BRIGHT_RED, :style = :BOLD])
	? colorText([:text = "Details: ", :color = :YELLOW]) + colorText([:text = cCatchError, :color = :CYAN])
done


func getRing2EXEContent
	return `aLibrary = [:name = :bcrypt,
	 :title = "Bcrypt",
	 :windowsfiles = [
		"ring_bcrypt.dll"
	 ],
